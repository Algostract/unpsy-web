...
function onUpdate(updatedValue: string | undefined) {
  if (updatedValue) value.value = updatedValue
}
</script>
...
<template>
  <div class="flex flex-col gap-2">
    ...
    <ul class="flex justify-between gap-2">
      <li v-for="gender in genders" :key="gender" class="flex gap-1">
    ...
    <span class="text-sm font-semi-bold text-alert-500">{{ errors[0] }}</span>
  </div>
</template>
