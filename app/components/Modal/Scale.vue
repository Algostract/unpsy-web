...
function onMove(_slide: unknown, list: { items: string[] }, _prev: unknown, curr: { page: number }) {
  ...
  <ModalBase :is-open="isOpen" inner-class="w-[700px] p-4 overflow-hidden text-black dark:text-white" @close="onClose">
    ...
    <Splide ref="splide" :options="dataSplideOption" tag="div" :has-track="false" @splide:pagination:updated="onMove">
      ...
      <div class="splide__arrows mt-4 flex h-8 justify-between">
        ...
        <button v-show="!isLastSlide" class="splide__arrow splide__arrow--next scale-[-1]">
          <NuxtIcon name="local:chevron-bold" />
        </button>
        <AppButton
          v-show="isLastSlide && !result"
          :is-loading="isLoading"
          title="Calculate"
          size="M"
          class="ml-auto !px-3 !py-1 transition-[width] duration-300 ease-in-out"
          @click="onCalculate(choices)" />
        <AppButton v-show="isLastSlide && result" title="Print" size="M" class="ml-auto !px-3 !py-1 transition-[width] duration-300 ease-in-out" @click="onPrint(choices)" />
      </div>
    </Splide>
  </ModalBase>
</template>
...
<style scoped>
:deep(.pagination) {
  @apply absolute bottom-2 left-1/2 flex h-[10px] -translate-x-1/2 gap-1;
}
