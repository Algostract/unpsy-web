...
function toggle(scale: string) {
  return selectedScales.value.has(scale) ? selectedScales.value.delete(scale) : selectedScales.value.add(scale)
}
...
<template>
  <ModelBase id="download" :is-open="isOpen" class="grid h-[375px] w-[500px] grid-cols-[repeat(2,auto)] grid-rows-[repeat(3,auto)] gap-6 text-black dark:text-white" @close="onClose">
    ...
    <div class="col-start-1 row-start-2 flex flex-wrap gap-3">
      <BaseChips v-for="{ name } in scales" :key="name" :title="name" class="cursor-pointer !px-4 !py-1" :class="{ 'bg-primary-400': selectedScales.has(name) }" @click="toggle(name)" />
    </div>
