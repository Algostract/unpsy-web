...
class SingleWeightedScale {
  ...
  score(responses: { index: number; value: boolean }[]): number {
    return this.subScales.reduce(
      (total, subScales) =>
        total +
        subScales.indexes.reduce(
          (total, index, indexIndex) =>
            total + (typeof subScales.weight == 'number' ? subScales.weight : subScales.weight[indexIndex]) * Number(subScales.condition ? responses[index - 1].value : !responses[index - 1].value),
          0
        ),
      0
    )
...
class PairScale {
  ...
  constructor({ TT = { indices: [], weight: 1 }, TF = { indices: [], weight: 1 }, FT = { indices: [], weight: 1 }, FF = { indices: [], weight: 1 } }, bias = 0) {
    this.TT = TT
    this.TF = TF
    this.FT = FT
    this.FF = FF
...
class CountScale {
  ...
  score(responses: { index: number; value: boolean | number }[]): number {
    return responses.slice(this.start - 1, this.start - 1 + this.count).reduce((sum, response) => sum + Math.abs(Number(this.inverse) - Number(response.value === this.label)), 0)
  }
}
...
export { SingleScale, SingleWeightedScale, PairScale, ShiftScale, CountScale, AverageScale, SumScale }
